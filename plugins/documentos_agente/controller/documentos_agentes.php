...
require_model('documento_agente.php');
 ...
 * @author carlos
 */
class documentos_agentes extends fs_controller
{
   public $documentos;
   ...
   private function upload_documento()
   {
      if( is_uploaded_file($_FILES['fdocumento']['tmp_name']) )
      {
         ...
         if( copy($_FILES['fdocumento']['tmp_name'], 'documentos/'.$nuevon) )
         {
            ...
            if($_GET['folder'] == 'agentes')
            {
               $doc->codagente = $_GET['id'];
            }
   ...
   private function get_documentos()
   {
      ...
      if($_GET['folder'] == 'agentes')
      {
         return $doc->all_from('codagente', $_GET['id']);
      }
   }
